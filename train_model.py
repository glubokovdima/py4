...
FEATURE_BLACKLIST = {
    "long": [], # Пример: ["stoch_k", "returns_rolling_std_20"]
    "short": [], 
    "tp_hit": []
}
...
N_CV_SPLITS = 5
# Значения по умолчанию для аргументов командной строки
...
def _train_binary_classifier(
    ...
    X_optuna_train_pool, y_optuna_train_pool = X_cv_pool_full, y_cv_pool_target
    X_val_for_threshold, y_val_for_threshold = pd.DataFrame(columns=X_cv_pool_full.columns), pd.Series(dtype='int')

    min_samples_for_val_split = N_CV_SPLITS * 20
    if len(X_cv_pool_full) > min_samples_for_val_split + (N_CV_SPLITS * 2):
        ...
        if val_threshold_size > 0:
            split_idx_cv = len(X_cv_pool_full) - val_threshold_size
            X_optuna_train_pool, y_optuna_train_pool = X_cv_pool_full.iloc[:split_idx_cv].copy(), y_cv_pool_target.iloc[:split_idx_cv].copy()
            X_val_for_threshold, y_val_for_threshold = X_cv_pool_full.iloc[split_idx_cv:].copy(), y_cv_pool_target.iloc[split_idx_cv:].copy()
            logging.info(f"CV пул разделен: Optuna/Train: {len(X_optuna_train_pool)}, Валидация порога: {len(X_val_for_threshold)}")
            if y_val_for_threshold.nunique() < 2 or X_val_for_threshold.empty:
                logging.warning("Валидация порога пуста/1 класс. Весь CV пул для Optuna/Train.")
                X_optuna_train_pool, y_optuna_train_pool = X_cv_pool_full.copy(), y_cv_pool_target.copy()
                X_val_for_threshold, y_val_for_threshold = pd.DataFrame(columns=X_cv_pool_full.columns), pd.Series(dtype='int')
        else: logging.warning("Не удалось выделить валидацию. Весь CV пул для Optuna/Train.")
    else: logging.warning("CV пул мал для выделения валидации. Весь CV пул для Optuna/Train.")
    
    selected_features = []
    if X_optuna_train_pool.empty or y_optuna_train_pool.empty or len(X_optuna_train_pool.columns) == 0:
        logging.warning(f"X_optuna_train_pool пуст/нет признаков для {target_name}. Пропуск отбора.")
        if not X_cv_pool_full.empty and len(X_cv_pool_full.columns) > 0 : selected_features = X_cv_pool_full.columns.tolist()
        ...
        # ... (код отбора признаков с TimeSeriesSplit, используя n_top_features)
        ...
        logging.info(f"🚀 Отбор Топ-{n_top_features} признаков для {model_log_prefix} на {len(X_optuna_train_pool)} образцах...")
        for fold, (train_idx, val_idx) in enumerate(tscv_fs.split(X_optuna_train_pool, y_optuna_train_pool)):
            ...
            if X_f.empty or y_f.empty or X_v.empty or y_v.empty or \
               y_f.nunique() < 2 or y_v.nunique() < 2:
                logging.warning(f"Фолд {fold+1} пропущен: недостаточно данных или классов в обучающей/валидационной выборке фолда.")
                continue
            clf_fi = CatBoostClassifier(iterations=200, learning_rate=0.05, early_stopping_rounds=25, loss_function='Logloss', eval_metric='F1', task_type="GPU", devices='0', random_seed=42, verbose=0)
            try:
            ...
            except Exception as e: logging.warning(f"Ошибка в фолде {fold+1} отбора {target_name}: {e}")
        if num_successful_folds > 0:
            fi_mean = pd.Series(feature_importances_fi / num_successful_folds, index=X_optuna_train_pool.columns).sort_values(ascending=False)
            ...
            selected_features = X_optuna_train_pool.columns.tolist()
            logging.warning(f"Отбор признаков для {target_name} не дал результатов. Все ({len(selected_features)}) используются.")

    if target_name in FEATURE_BLACKLIST:
        ...
        if current_blacklist:
            ...
            if len(selected_features) < original_len:
                logging.info(f"Исключено по черному списку для {model_log_prefix}: {original_len - len(selected_features)} шт. Осталось: {len(selected_features)}")
            if not selected_features and original_len > 0 :
                logging.warning(f"После черного списка для {model_log_prefix} не осталось признаков! Используется исходный набор.")
    ...
    X_val_for_threshold_sel = X_val_for_threshold[selected_features] if not X_val_for_threshold.empty else pd.DataFrame(columns=selected_features)
    
    try:
        features_list_path = f"models/{file_prefix}_features_{target_name}_selected.txt"
        with open(features_list_path, "w", encoding="utf-8") as f: json.dump(selected_features, f, indent=2)
        logging.info(f"Отобранные признаки для {model_log_prefix} сохранены в {features_list_path}")
    except Exception as e: logging.error(f"Ошибка сохранения списка признаков: {e}")
    ...
    def objective(trial):
        ...
        total_trial = sum(counts_trial.values())
        num_classes_trial = len(counts_trial) if len(counts_trial) > 0 else 1
        class_weights_objective = {}
        minority_label_obj = 1 if counts_trial.get(1,0) < counts_trial.get(0,0) else (0 if counts_trial.get(0,0) < counts_trial.get(1,0) else -1)
        for cls_label, count_val in counts_trial.items():
            base_weight = total_trial / (num_classes_trial * count_val) if count_val > 0 else 1.0
            class_weights_objective[cls_label] = base_weight * minority_weight_boost_factor if cls_label == minority_label_obj and minority_label_obj != -1 else base_weight
        if 0 not in class_weights_objective: class_weights_objective[0] = 1.0 * (minority_weight_boost_factor if 0 == minority_label_obj and minority_label_obj !=-1 else 1.0)
        if 1 not in class_weights_objective: class_weights_objective[1] = 1.0 * (minority_weight_boost_factor if 1 == minority_label_obj and minority_label_obj !=-1 else 1.0)
        class_weights_list_objective = [class_weights_objective.get(0, 1.0), class_weights_objective.get(1, 1.0)]
        params = { # ... (параметры для Optuna) ...
                  ...
                  'loss_function': 'Logloss', 'eval_metric': 'F1', 'early_stopping_rounds': 50, 
                  'random_seed': 42, 'task_type': 'GPU', 'devices': '0', 'verbose': 0,
                  'class_weights': class_weights_list_objective}
        tscv_optuna = TimeSeriesSplit(n_splits=N_CV_SPLITS)
        try:
            cv_results = cv(Pool(X_optuna_train_sel, y_optuna_train_pool.astype(int), feature_names=list(X_optuna_train_sel.columns)), params, folds=tscv_optuna, plot=False)
            ...
            logging.error(f"  Ошибка в Optuna Попытке {trial_counter} во время CV: {e_cv}\n  Параметры: {trial.params}")
            return -1.0
    ...
    try:
        ...
        logging.info(f"🎯 Лучшие параметры Optuna для {model_log_prefix}: {best_params_optuna}, F1_cv_score: {study.best_value:.4f}")
    except Exception as e:
        logging.warning(f"Optuna для {model_log_prefix} завершилась с ошибкой: {e}. Используются параметры по умолчанию.")
        best_params_optuna = {}
    ...
    total_final_train = sum(counts_final_train.values())
    num_classes_final_train = len(counts_final_train) if len(counts_final_train) > 0 else 1
    class_weights_final_fit = {}
    minority_label_final = 1 if counts_final_train.get(1,0) < counts_final_train.get(0,0) else (0 if counts_final_train.get(0,0) < counts_final_train.get(1,0) else -1)
    for cls_label, count_val in counts_final_train.items():
        ...
        class_weights_final_fit[cls_label] = base_weight * minority_weight_boost_factor if cls_label == minority_label_final and minority_label_final != -1 else base_weight
    if 0 not in class_weights_final_fit: class_weights_final_fit[0] = 1.0 * (minority_weight_boost_factor if 0 == minority_label_final and minority_label_final !=-1 else 1.0)
    if 1 not in class_weights_final_fit: class_weights_final_fit[1] = 1.0 * (minority_weight_boost_factor if 1 == minority_label_final and minority_label_final !=-1 else 1.0)
    ...
    if target_name == "tp_hit":
        ...
        if len(counts_tp_hit_train) == 2 and counts_tp_hit_train.get(1,0) >= counts_tp_hit_train.get(0,0):
            logging.info(f"Для {model_log_prefix} класс 1 не миноритарный ({counts_tp_hit_train.to_dict()}). SMOTE пропущен.")
            apply_smote_final = False
            
    if apply_smote_final:
        ...
        if y_train_for_final_model_target.nunique() < 2:
            logging.warning(f"Пропуск SMOTE для {target_name}: только один класс в y_train_for_final_model_target.")
        elif min_class_count <= 1:
            logging.warning(f"Пропуск SMOTE для {target_name}: в минорном классе слишком мало примеров ({min_class_count}). Распределение: {value_counts_before_smote}")
        else:
            k_val = max(1, min(4, min_class_count - 1)) 
            smote = SMOTE(random_state=42, k_neighbors=k_val)
            logging.info(f"Применение SMOTE к финальным обучающим данным для {model_log_prefix} с k_neighbors={k_val}. Распределение до: {value_counts_before_smote}")
            try:
                X_train_for_final_model_sel, y_train_for_final_model_target = smote.fit_resample(X_train_for_final_model_sel, y_train_for_final_model_target)
                logging.info(f"Размер после SMOTE для {target_name}: {pd.Series(y_train_for_final_model_target).value_counts().to_dict()}")
            except Exception as e:
                logging.warning(f"SMOTE для {target_name} не удался: {e}. Обучение на оригинальных данных.")
    
    logging.info(f"\n🚀 Обучение финальной модели {model_log_prefix}...")
    final_model_uncalibrated = CatBoostClassifier(**final_model_params)
    eval_set_for_final_fit = (X_val_for_threshold_sel, y_val_for_threshold.astype(int)) \
        if not X_val_for_threshold_sel.empty and not y_val_for_threshold.empty and y_val_for_threshold.nunique() >= 2 \
        else None
    if eval_set_for_final_fit is None:
        logging.warning(f"Нет валидационного набора для early stopping финальной модели {model_log_prefix}.")

    if not X_train_for_final_model_sel.empty and not y_train_for_final_model_target.empty:
        try:
            final_model_uncalibrated.fit(X_train_for_final_model_sel, y_train_for_final_model_target.astype(int), eval_set=eval_set_for_final_fit, plot=False)
        except Exception as e:
            logging.error(f"Ошибка обучения финальной модели {model_log_prefix}: {e}", exc_info=True)
            return None, selected_features, 0.5
    else:
        logging.error(f"Обучение {model_log_prefix} невозможно: финальные X или y пусты.")
        return None, selected_features, 0.5

    final_model_calibrated = final_model_uncalibrated
    if target_name in ["tp_hit", "long", "short"]:
        if not X_val_for_threshold_sel.empty and not y_val_for_threshold.empty and y_val_for_threshold.nunique() >= 2:
            logging.info(f"Калибровка вероятностей для {model_log_prefix}...")
            try:
                calibrated_clf = CalibratedClassifierCV(final_model_uncalibrated, method='isotonic', cv=None)
                calibrated_clf.fit(X_val_for_threshold_sel, y_val_for_threshold.astype(int))
                final_model_calibrated = calibrated_clf
                logging.info(f"Калибровка для {model_log_prefix} завершена.")
            except Exception as e:
                logging.warning(f"Ошибка калибровки для {model_log_prefix}: {e}. Используется некалиброванная модель.")
        else:
            logging.warning(f"Пропуск калибровки для {model_log_prefix}: нет валидационных данных.")
            
    optimal_threshold = 0.5
    if not X_val_for_threshold_sel.empty and not y_val_for_threshold.empty and y_val_for_threshold.nunique() >= 2:
        optimal_threshold = find_optimal_threshold(final_model_calibrated, X_val_for_threshold_sel, y_val_for_threshold.astype(int))
    else:
        logging.warning(f"Невозможно подобрать оптимальный порог для {model_log_prefix}, используется 0.5.")

    if not X_val_for_threshold_sel.empty and not y_val_for_threshold.empty and y_val_for_threshold.nunique() >= 2:
        logging.info(f"Расчет Permutation Importance для {model_log_prefix}...")
        try:
            X_perm_eval = X_test_full[selected_features] 
            y_perm_eval = y_test_target                 

            if X_perm_eval.empty or y_perm_eval.empty or y_perm_eval.nunique() < 2:
                logging.warning(f"Недостаточно данных в X_test_full для Permutation Importance {model_log_prefix}. Используется X_val_for_threshold_sel.")
                X_perm_eval = X_val_for_threshold_sel
                y_perm_eval = y_val_for_threshold
            
            if not X_perm_eval.empty and not y_perm_eval.empty and y_perm_eval.nunique() >= 2:
                perm_importance_result = permutation_importance(final_model_calibrated, X_perm_eval, y_perm_eval.astype(int),
                ...
                logging.warning(f"Данные для Permutation Importance {model_log_prefix} все еще невалидны. Пропуск.")
        except Exception as e:
...
def train_all_models(tf_train, symbol=None, cli_args=None):
    ...
    try:
        ...
        if 'timestamp' in df.columns: df = df.sort_values(by='timestamp').reset_index(drop=True); logging.info(f"Данные из {features_path} отсортированы.")
    except Exception as e: logging.error(f"Ошибка чтения {features_path}: {e}"); return
    ...
    if any(t not in df.columns for t in required_targets_for_models): logging.error(f"Отсутствуют таргеты в {features_path}."); return
    ...
    if target_tp_hit_col in df.columns:
        if not df_cleaned[target_tp_hit_col].isnull().all() and df_cleaned[target_tp_hit_col].nunique(dropna=True) >= 2:
            ...
            if tp_hit_counts.min() >= N_CV_SPLITS : train_tp_hit_model_flag = True; logging.info(f"'{target_tp_hit_col}'. Распределение: {tp_hit_counts.to_dict()}")
            else: logging.warning(f"'{target_tp_hit_col}' мало примеров ({tp_hit_counts.to_dict()}). TP-hit не будет обучен.")
        else: logging.warning(f"'{target_tp_hit_col}' все NaN или 1 класс. TP-hit не будет обучен.")
    else: logging.warning(f"'{target_tp_hit_col}' не найден. TP-hit не будет обучен.")
    ...
    if train_tp_hit_model_flag and target_tp_hit_col in df_cleaned.columns: excluded_cols_for_features.append(target_tp_hit_col)
    potential_leak_cols = ['target_class', 'target_up']
    for col in potential_leak_cols:
        if col in df_cleaned.columns and col not in excluded_cols_for_features: excluded_cols_for_features.append(col); logging.warning(f"'{col}' исключена.")
    ...
    logging.info(f"Начальное кол-во признаков ({log_context_str}): {len(feature_cols_initial)}. Первые 5: {feature_cols_initial[:5]}")
    ...
    y_tp_hit_all = df_cleaned[target_tp_hit_col].astype(int) if train_tp_hit_model_flag and target_tp_hit_col in df_cleaned.columns else None

    min_data_needed_for_robust_split = N_CV_SPLITS * 10 
    if len(X_all_data) < min_data_needed_for_robust_split : logging.warning(f"Данных ({len(X_all_data)}) может быть мало.")
    ...
    if actual_test_size < min_test_samples:
        if len(X_all_data) > min_test_samples * 2: actual_test_size = min_test_samples; logging.warning(f"Тестовая выборка мала. Используется {actual_test_size}.")
        else: logging.error(f"Мало данных ({len(X_all_data)}) для теста. Прервано."); return
    ...
    if not (cli_args and cli_args.skip_long):
        clf_long_model, features_long, opt_thresh_long = _train_binary_classifier(X_cv_pool, y_long_cv_pool, X_test_full, y_test_long, "long", log_context_str, file_prefix, n_top_features_to_use, optuna_trials_clf_to_use, minority_boost_to_use)
        ...
        if os.path.exists(model_path) and os.path.exists(features_path_txt):
            try:
                ...
                if os.path.exists(threshold_path): opt_thresh_long = json.load(open(threshold_path, "r")).get("long", 0.5)
                logging.info(f"Загружена clf_long. Порог: {opt_thresh_long}")
                if clf_long_model: optimal_thresholds["long"] = opt_thresh_long
            except Exception as e: logging.error(f"Ошибка загрузки clf_long: {e}."); clf_long_model, features_long, opt_thresh_long = None, [], 0.5
        else: logging.warning(f"Пропуск clf_long, модель/признаки не найдены.")
    
    if not (cli_args and cli_args.skip_short):
        clf_short_model, features_short, opt_thresh_short = _train_binary_classifier(X_cv_pool, y_short_cv_pool, X_test_full, y_test_short, "short", log_context_str, file_prefix, n_top_features_to_use, optuna_trials_clf_to_use, minority_boost_to_use)
        ...
        if os.path.exists(model_path) and os.path.exists(features_path_txt):
            try:
                ...
                if os.path.exists(threshold_path): opt_thresh_short = json.load(open(threshold_path, "r")).get("short", 0.5)
                logging.info(f"Загружена clf_short. Порог: {opt_thresh_short}")
                if clf_short_model: optimal_thresholds["short"] = opt_thresh_short
            except Exception as e: logging.error(f"Ошибка загрузки clf_short: {e}."); clf_short_model, features_short, opt_thresh_short = None, [], 0.5
        else: logging.warning(f"Пропуск clf_short, модель/признаки не найдены.")
    ...
    if not features_long: 
        ...
        if os.path.exists(features_long_path_check):
            try:
                with open(features_long_path_check, "r") as f: features_long = json.load(f)
                logging.info(f"Загружен список признаков clf_long (обучение было пропущено).")
            except Exception as e: logging.error(f"Не удалось загрузить признаки clf_long: {e}")
        else: logging.warning("Обучение clf_long пропущено, файл признаков не найден.")
            
    active_feature_set_for_others = features_long if features_long else feature_cols_initial
    if not active_feature_set_for_others: logging.error(f"Нет признаков для регрессоров/TP-hit для {log_context_str}.")
    else:
        X_cv_pool_others_sel, X_test_others_sel = X_cv_pool[active_feature_set_for_others], X_test_full[active_feature_set_for_others]
        if not (cli_args and cli_args.skip_regressors):
            def objective_regressor(trial, X_tr, y_tr, X_v, y_v, name, trial_num, total_trials):
                ...
                try:
                    ...
                    logging.error(f"  Ошибка в Optuna Попытке {trial_num} для {name}: {e_cv_r}\n  Параметры: {trial.params}")
                    return float('inf')
            ...
            if len(X_cv_pool_others_sel) - val_size_reg >= N_CV_SPLITS * 5 : 
                ...
                study_d.optimize(lambda t: objective_regressor(t, X_tr_r_opt, y_d_tr_r_opt, X_v_r_opt, y_d_v_r_opt, "reg_delta", t.number+1, optuna_trials_reg_to_use),
                                 n_trials=optuna_trials_reg_to_use, n_jobs=1, callbacks=[EarlyStoppingCallback(patience=7, min_delta=1e-5)])
                bp_d = study_d.best_params
                reg_delta_model = CatBoostRegressor(**bp_d, loss_function='RMSE', eval_metric='MAE', random_seed=42, task_type="GPU", devices='0', verbose=100, early_stopping_rounds=30)
                reg_delta_model.fit(X_cv_pool_others_sel, y_delta_cv_pool, eval_set=(X_test_others_sel, y_test_delta) if not X_test_others_sel.empty and not y_test_delta.empty else None)
                ...
                study_v.optimize(lambda t: objective_regressor(t, X_tr_r_opt, y_v_tr_r_opt, X_v_r_opt, y_v_v_r_opt, "reg_vol", t.number+1, optuna_trials_reg_to_use),
                                 n_trials=optuna_trials_reg_to_use, n_jobs=1, callbacks=[EarlyStoppingCallback(patience=7, min_delta=1e-5)])
                bp_v = study_v.best_params
                reg_vol_model = CatBoostRegressor(**bp_v, loss_function='RMSE', eval_metric='MAE', random_seed=42, task_type="GPU", devices='0', verbose=100, early_stopping_rounds=30)
                reg_vol_model.fit(X_cv_pool_others_sel, y_vol_cv_pool, eval_set=(X_test_others_sel, y_test_vol) if not X_test_others_sel.empty and not y_test_vol.empty else None)
                logging.info(f"Лучшие параметры reg_vol: {bp_v}, MAE_val_optuna: {study_v.best_value:.6f}")
            else: # Дефолтные для регрессоров
                logging.warning("Мало данных для Optuna HPO регрессоров. Дефолтные параметры.")
                reg_delta_model = CatBoostRegressor(iterations=500,learning_rate=0.03,depth=6,loss_function='RMSE',eval_metric='MAE',task_type="GPU",devices='0',random_seed=42,verbose=100,early_stopping_rounds=50)
                if not X_cv_pool_others_sel.empty and not y_delta_cv_pool.empty: reg_delta_model.fit(X_cv_pool_others_sel, y_delta_cv_pool, eval_set=(X_test_others_sel, y_test_delta) if not X_test_others_sel.empty and not y_test_delta.empty else None)
                reg_vol_model = CatBoostRegressor(iterations=500,learning_rate=0.03,depth=6,loss_function='RMSE',eval_metric='MAE',task_type="GPU",devices='0',random_seed=42,verbose=100,early_stopping_rounds=50)
                if not X_cv_pool_others_sel.empty and not y_vol_cv_pool.empty: reg_vol_model.fit(X_cv_pool_others_sel, y_vol_cv_pool, eval_set=(X_test_others_sel, y_test_vol) if not X_test_others_sel.empty and not y_test_vol.empty else None)
        else: # Загрузка регрессоров
            logging.info(f"Пропуск обучения регрессоров для {log_context_str}.")
            if os.path.exists(f"models/{file_prefix}_reg_delta.pkl"): reg_delta_model = joblib.load(f"models/{file_prefix}_reg_delta.pkl")
            if os.path.exists(f"models/{file_prefix}_reg_vol.pkl"): reg_vol_model = joblib.load(f"models/{file_prefix}_reg_vol.pkl")
            if reg_delta_model and reg_vol_model: logging.info("Загружены существующие модели регрессоров.")
            else: logging.warning("Пропуск регрессоров, но существующие модели не найдены.")
        ...
        if not (cli_args and cli_args.skip_tp_hit):
            if train_tp_hit_model_flag and y_tp_hit_cv_pool is not None:
                if y_tp_hit_cv_pool.nunique() >= 2 and y_tp_hit_cv_pool.value_counts().min() >= N_CV_SPLITS:
                    clf_tp_hit_model, _, opt_thresh_tp_hit = _train_binary_classifier(X_cv_pool_others_sel, y_tp_hit_cv_pool, X_test_others_sel, y_test_tp_hit, "tp_hit", log_context_str, file_prefix, n_top_features_to_use, optuna_trials_clf_to_use, minority_boost_to_use)
                    if clf_tp_hit_model: optimal_thresholds["tp_hit"] = opt_thresh_tp_hit
                else: logging.warning(f"Мало данных/классов для clf_tp_hit. Пропуск.")
            ...
            if os.path.exists(model_path):
                ...
                if os.path.exists(threshold_path): opt_thresh_tp_hit = json.load(open(threshold_path, "r")).get("tp_hit", 0.5)
                logging.info(f"Загружена clf_tp_hit. Порог: {opt_thresh_tp_hit}")
                if clf_tp_hit_model: optimal_thresholds["tp_hit"] = opt_thresh_tp_hit
            else: logging.warning("Пропуск clf_tp_hit, модель не найдена.")
            
    thresholds_path = f"models/{file_prefix}_optimal_thresholds.json"
    try:
        # Перед сохранением убедимся, что файл существует и содержит словарь, или создаем новый
        existing_thresholds = {}
        if os.path.exists(thresholds_path):
            try:
                with open(thresholds_path, "r", encoding="utf-8") as f:
                    existing_thresholds = json.load(f)
                if not isinstance(existing_thresholds, dict): # Если файл не JSON-словарь
                    existing_thresholds = {}
            except json.JSONDecodeError: # Если файл не JSON
                existing_thresholds = {}
                logging.warning(f"Файл {thresholds_path} поврежден или не JSON, будет перезаписан.")
        
        existing_thresholds.update(optimal_thresholds) # Обновляем только те ключи, что есть в optimal_thresholds

        with open(thresholds_path, "w", encoding="utf-8") as f: json.dump(existing_thresholds, f, indent=2)
        logging.info(f"Оптимальные пороги сохранены/обновлены в {thresholds_path}: {existing_thresholds}")
    except Exception as e: logging.error(f"Ошибка сохранения файла с порогами {thresholds_path}: {e}")

    logging.info(f"\n💾 Сохранение моделей для {log_context_str}...")
    if clf_long_model: joblib.dump(clf_long_model, f"models/{file_prefix}_clf_long.pkl"); logging.info("Модель clf_long сохранена.")
    if clf_short_model: joblib.dump(clf_short_model, f"models/{file_prefix}_clf_short.pkl"); logging.info("Модель clf_short сохранена.")
    if reg_delta_model: joblib.dump(reg_delta_model, f"models/{file_prefix}_reg_delta.pkl"); logging.info("Модель reg_delta сохранена.")
    if reg_vol_model: joblib.dump(reg_vol_model, f"models/{file_prefix}_reg_vol.pkl"); logging.info("Модель reg_vol сохранена.")
    if clf_tp_hit_model: joblib.dump(clf_tp_hit_model, f"models/{file_prefix}_clf_tp_hit.pkl"); logging.info("Модель clf_tp_hit сохранена.")

    metrics_output = f"\n📊 Метрики на тестовой выборке ({log_context_str}):\n"
    def get_classification_metrics_with_threshold(model, X_eval, y_eval, model_name, threshold, class_names_map={0:'NO', 1:'YES'}):
        # ... (код функции get_classification_metrics_with_threshold как был) ...
        ...
        try:
            ...
            if 1 not in model_classes_list:
                logging.warning(f"Класс 1 отсутствует в модели {model_name}. Метрики для класса 1 могут быть некорректны.")
            ...
            if len(present_labels) < 2:
                ...
                out_str += classification_report(y_eval, y_pred, labels=present_labels, target_names=target_names_for_report, digits=4, zero_division=0) + "\n"
                out_str += f"   Accuracy: {accuracy_score(y_eval, y_pred):.4f}\n"
            ...
            if len(cm_labels) >=2 : 
                ...
                cm_df = pd.DataFrame(cm, index=[f"True_{n}" for n in cm_target_names], columns=[f"Pred_{n}" for n in cm_target_names])
                out_str += f"   Confusion Matrix:\n{cm_df.to_string()}\n"
            else: out_str += "   Confusion Matrix: N/A (менее 2 классов для матрицы)\n"
        except Exception as e: out_str += f"Ошибка метрик для {model_name}: {e}\n"
        return out_str

    if clf_long_model and features_long and not y_test_long.empty:
        X_test_long_sel = X_test_full[features_long] if features_long and all(f in X_test_full.columns for f in features_long) else pd.DataFrame()
        if not X_test_long_sel.empty: metrics_output += get_classification_metrics_with_threshold(clf_long_model, X_test_long_sel, y_test_long, "Классификатор Long", optimal_thresholds.get("long", 0.5), {0:'NO_LONG', 1:'LONG'})
        else: metrics_output += "--- Классификатор Long ---\nПропущено из-за отсутствия признаков в тесте.\n"
    if clf_short_model and features_short and not y_test_short.empty:
        X_test_short_sel = X_test_full[features_short] if features_short and all(f in X_test_full.columns for f in features_short) else pd.DataFrame()
        if not X_test_short_sel.empty: metrics_output += get_classification_metrics_with_threshold(clf_short_model, X_test_short_sel, y_test_short, "Классификатор Short", optimal_thresholds.get("short", 0.5), {0:'NO_SHORT', 1:'SHORT'})
        else: metrics_output += "--- Классификатор Short ---\nПропущено из-за отсутствия признаков в тесте.\n"
    ...
    if active_feature_set_for_others and not X_test_others_sel.empty:
        ...
        if clf_tp_hit_model and y_test_tp_hit is not None and not y_test_tp_hit.empty: metrics_output += get_classification_metrics_with_threshold(clf_tp_hit_model, X_test_others_sel, y_test_tp_hit, "Классификатор TP-Hit", optimal_thresholds.get("tp_hit", 0.5), {0:'NO_TP_HIT', 1:'TP_HIT'})
...
if __name__ == "__main__":
    ...
    if args.symbol_group:
        ...
        if args.symbol.lower() in SYMBOL_GROUPS: 
            ...
            entities_to_process = [args.symbol_group]
            is_group_run = True
            logging.info(f"🧩 Аргумент --symbol ('{args.symbol}') распознан как группа. Обучение для группы: {args.symbol_group}")
            args.symbol = None 
        else:
    ...
    for entity_name in entities_to_process:
        ...
        if not (args.skip_long and args.skip_short and args.skip_regressors and args.skip_tp_hit):
            try:
                with open(current_main_log_path, "w", encoding="utf-8") as f_clear:
                    f_clear.write(f"=== Начало лога обучения для {current_log_context_str} ({pd.Timestamp.now()}) ===\n\n")
            except Exception as e:
        ...
        try:
            ...
            if len(entities_to_process) == 1: sys.exit(1)
            else: logging.warning(f"Продолжение со следующим после ошибки на {current_log_context_str}.")